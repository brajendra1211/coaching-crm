...
@section('page_subtitle', $examSeries->title)
...
@php $selected = $examSeries->exams->pluck('pivot.unlock_rule', 'id')->toArray(); @endphp
<div class="card">
    <div class="card-header" style="display:flex;justify-content:space-between;gap:14px;align-items:center;flex-wrap:wrap;"><div><h3>{{ $examSeries->title }}</h3><p class="muted" style="margin:6px 0 0;">Reusable exams select karke series build karein.</p></div><a href="{{ route('teacher.series.index') }}" class="btn btn-light">Back</a></div>
    ...
    <form method="POST" action="{{ route('teacher.series.builder.update', $examSeries) }}">
        @csrf @method('PUT')
        <div class="builder-grid">
            <aside class="builder-panel">
                ...
                <div class="muted" style="font-size:14px;line-height:1.7;margin-top:8px;">
                    ...
                    <div>Access: {{ ucfirst($examSeries->access_type) }}</div>
                    <div>Selected exams: {{ count($selected) }}</div>
                </div>
                <button class="btn btn-primary" type="submit" style="width:100%;margin-top:14px;">Save Series</button>
            </aside>
            <section>
                @forelse($exams as $exam)
                    <label class="exam-row">
                        <input type="checkbox" name="exam_ids[]" value="{{ $exam->id }}" {{ array_key_exists($exam->id, $selected) ? 'checked' : '' }}>
                        <span><strong>{{ $exam->title }}</strong><span class="meta"><span class="tag">{{ $exam->batch->name ?? 'All' }}</span><span class="tag">{{ $exam->label ?: ucfirst($exam->difficulty) }}</span><span class="tag">{{ ucfirst($exam->access_type) }}</span><span class="tag">{{ $exam->total_marks }} marks</span></span></span>
                        <select name="unlock_rules[{{ $exam->id }}]"><option value="always" {{ ($selected[$exam->id] ?? 'always') === 'always' ? 'selected' : '' }}>Always Open</option><option value="after_previous" {{ ($selected[$exam->id] ?? '') === 'after_previous' ? 'selected' : '' }}>After Previous</option><option value="scheduled_only" {{ ($selected[$exam->id] ?? '') === 'scheduled_only' ? 'selected' : '' }}>Scheduled Only</option></select>
                    </label>
                @empty
